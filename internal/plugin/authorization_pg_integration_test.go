...
func TestPostgresAuthorizationAndSecretRoundTrip(t *testing.T) {
	...
	defer pool.Close()
	if _, err := pool.Exec(ctx, `INSERT INTO users(id,username,nickname,email) VALUES(42,'plugin_test_user','插件测试','plugin-test@example.invalid') ON CONFLICT(id) DO NOTHING`); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.SetAdminGrant(ctx, version.ID, "schedule.self.read", "granted", "integration", map[string]interface{}{"scope": "self"}, "42", nil); err != nil {
		t.Fatal(err)
	}
	if _, err := service.SetUserConsent(ctx, "42", version.ID, "schedule.self.read", "granted", map[string]interface{}{"scope": "self"}); err != nil {
		t.Fatal(err)
	}
	if result := service.Authorize(ctx, AuthorizationInput{PluginName: record.Name, CapabilityCode: "schedule.self.read", OperationCode: "integration.read", ActorUserID: "42", ResourceOwnerID: "42", ResourceScope: map[string]interface{}{"scope": "self"}}); !result.Allow {
		t.Fatalf("authorization denied: %+v", result)
	}
	if result := service.Authorize(ctx, AuthorizationInput{PluginName: record.Name, CapabilityCode: "user.contact.read", OperationCode: "integration.undeclared", ActorUserID: "42", ResourceOwnerID: "42"}); result.ReasonCode != ReasonCapabilityNotDeclared {
		t.Fatalf("undeclared reason=%s", result.ReasonCode)
	}
	decisions, err := service.Decisions(ctx, record.Name, 10)
	if err != nil || len(decisions) < 2 {
		t.Fatalf("decision audit len=%d err=%v", len(decisions), err)
	}
	...
	owner := int64(42)
	if _, err := secrets.Put(ctx, record.ID, &owner, "mail.password", "first", &owner); err != nil {
		t.Fatal(err)
	}
	if _, err := secrets.Put(ctx, record.ID, &owner, "mail.password", "second", &owner); err != nil {
		t.Fatalf("secret rotation: %v", err)
	}
	if value, err := secrets.Resolve(ctx, record.ID, &owner, "mail.password"); err != nil || value != "second" {
		t.Fatalf("secret resolve value=%q err=%v", value, err)
	}
}
