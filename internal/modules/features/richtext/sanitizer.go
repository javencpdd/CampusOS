...
func Sanitize(input string) (SanitizeResult, error) {
	...
	if trimmed == "" {
		return SanitizeResult{}, fmt.Errorf("%w: content_html is required", ErrInvalidArticle)
	}
	if len(trimmed) > MaxHTMLBytes {
		return SanitizeResult{}, fmt.Errorf("%w: content_html must not exceed %d bytes", ErrInvalidArticle, MaxHTMLBytes)
	}
	...
	if err != nil {
		return SanitizeResult{}, fmt.Errorf("%w: html cannot be parsed", ErrInvalidArticle)
	}
	ctx := &sanitizeContext{}
	...
	if ctx.nodeLimitHit {
		return SanitizeResult{}, fmt.Errorf("%w: html must not contain more than %d nodes", ErrInvalidArticle, MaxNodes)
	}
	if ctx.depthLimitHit {
		return SanitizeResult{}, fmt.Errorf("%w: html nesting depth must not exceed %d", ErrInvalidArticle, MaxDepth)
	}
	clean := strings.TrimSpace(out.String())
	plain := normalizePlainText(text.String())
	if plain == "" {
		return SanitizeResult{}, fmt.Errorf("%w: article body is required", ErrInvalidArticle)
	}
...
func containsDangerousValue(value string) bool {
	...
	blocked := []string{"<script", "</script", "javascript:", "vbscript:", "data:text/html", "\x00"}
...
func safeStyle(value string) bool {
	...
	blocked := []string{"url(", "expression(", "@import", "javascript:", "vbscript:", "data:", "behavior:", "-moz-binding"}
	for _, marker := range blocked {
		if strings.Contains(lower, marker) {
			return false
		}
