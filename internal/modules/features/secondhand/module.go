...
const portStore = "feature.secondhand.adapter.store"

type ModuleConfig struct {
	Enabled func() bool
}

// Module composes secondhand information through Community's public content
// ports. It owns only secondhand_details and cannot bypass Community
// moderation, category type policy, revision, or Thread data ownership.
type Module struct {
	config    ModuleConfig
...
func (m *Module) Register(app *platformmodule.AppContext) error {
	if app == nil {
		return errors.New("secondhand module app context is required")
	}
	storeValue, ok := app.Lookup(portStore)
	if !ok {
		return errors.New("secondhand store adapter is not bound by profile")
	}
	store, ok := storeValue.(Store)
	if !ok {
		return fmt.Errorf("secondhand store adapter has incompatible type %T", storeValue)
	}
...
func (m *Module) Start(context.Context) error {
	if m.app == nil || m.store == nil || m.community == nil || m.query == nil {
		return errors.New("secondhand module is not registered")
	}
...
func (m *Module) Health(context.Context) platformmodule.Health {
	if m.service == nil || m.handler == nil {
		return platformmodule.Health{Status: platformmodule.HealthUnhealthy, Message: "secondhand service is not started"}
	}
