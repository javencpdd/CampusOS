...
func (s *Service) ListPublic(ctx context.Context, filter domain.ThreadListFilter) ([]*Result, int64, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("load mutual aid details: %w", normalizeStoreError(err))
	}
	...
	for _, thread := range threads {
		...
		if !ok || detail == nil {
			...
			return nil, 0, fmt.Errorf("load mutual aid detail for thread %s: %w", thread.ID, ErrNotFound)
		}
