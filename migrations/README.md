...
此前 `000001`–`000011` 是开发阶段逐段演进记录。由于项目所有者已明确当前数据均可清空，它们已被合并为上述 `000001_v1_1_schema_baseline`，因此 `000008`–`000011` 不再独立存在。
...
`up` 会核对已执行文件的名称和 SHA-256；`down` 只回滚当前最高版本。由于当前只有一个 baseline，development/test 中执行一次 `down` 会移除全部应用表与参考数据，但保留两个 migration 元数据表。
