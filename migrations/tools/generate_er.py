...
默认输出：
    docs/architecture/database-er/CampusOS数据库ER图.png
    docs/architecture/database-er/CampusOS数据库实体关系说明.md
    ...
    for statement in statement_pattern.finditer(sql):
        ...
        for action in split_top_level(statement.group(2)):
            ...
            if drop_constraint:
                ...
                continue
            column_action = re.match(r'ADD\s+COLUMN\s+(.+)$', action.strip(), re.I | re.S)
            if column_action:
...
def main() -> int:
    ...
    ap.add_argument('--out', type=Path, default=Path('docs/architecture/database-er'), help='输出目录')
    ...
    (args.out / markdown_name).write_text(markdown, encoding='utf-8')
