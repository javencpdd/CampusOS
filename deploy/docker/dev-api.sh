...
source_fingerprint() {
  {
    ...
    # Some test fixtures use ignored data/ directories that can be owned by a
    # different container UID. They are never compilation inputs, so prune
    # them along with per-package .campusos state before scanning Go source.
    find cmd internal pkg sdk -type d \( -name '.campusos' -o -name data \) -prune -o -type f -name '*.go' -print0
