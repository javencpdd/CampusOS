  ...
  <section class="pack-manager" aria-labelledby="homepage-pack-heading">
    <div class="section-heading">
      <div>
        ...
        <p>
          管理员统一选择首页资源包。导入的 zip
          必须先通过服务端筛查，应用失败时可以回滚。
        </p>
      </div>
      <el-button
        type="warning"
    ...
    <div class="pack-controls">
      ...
      >
        ...
        >
          <span :data-homepage-pack-name="pack.name">
      ...
      <el-button
        ...
        >导入并应用</el-button
      >
      <el-button :loading="downloadingExample" @click="downloadExample"
        >导出当前示例</el-button
      >
    </div>
...
const loadPacks = async () => {
  ...
  try {
    ...
    if (
      !packs.value.some(
        (pack) => pack.name === selectedPack.value && pack.validation?.valid,
      )
    ) {
      selectedPack.value =
        packs.value.find((pack) => pack.validation?.valid)?.name || "";
    }
  } catch (error: any) {
...
const validatePack = async () => {
  ...
  try {
    validation.value = unwrap(
      await homeStylePackApi.validate(packFile.value),
    )?.validation;
  } catch (error: any) {
