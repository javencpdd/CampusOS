...
数据库既包含已验证的核心 PostgreSQL 外键，也保留部分由逻辑归属、索引和服务层约束表达的插件关系。修改 schema 前必须运行 `make database-check` 和 migration 验证；当前顺序 migration 为 `000001` 至 `000043`。`000039` 记录管理员准入状态变更，`000040` 增加服务端 MFA Session 强度、TOTP 加密信封、Ticket/恢复码摘要和管理员策略，`000041` 删除九个被同谓词复合索引严格左前缀覆盖的窄索引并启用 Schema 冗余门禁，`000042` 增加按用户覆盖的 User Storage 配额授权，`000043` 为回复保存父楼层号快照。详见 [数据库迁移与 Schema 冗余治理](/operations/database-migration-hygiene)。
