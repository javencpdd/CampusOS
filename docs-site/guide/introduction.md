...
项目当前开发基线为 `v0.6.26-dev`。v6 计划内工作已完成并封版，v7 已进入模块化单体渐进迁移：首阶段建立 Module Registry、依赖拓扑、健康状态、失败回滚和逆序停止，并接管 EventBus 与 Plugin Platform 基础设施生命周期。现有 API、数据库、插件和前端合同保持兼容。
