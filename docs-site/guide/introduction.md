...
项目当前开发基线为 `v0.11.0`。v7-v9 建立了模块化单体、功能域边界、Manifest v2、受管插件数据和用户授权；v10 完成内容治理状态机、统一内容查询、稳定权限 Code、路由操作目录、授权审计、扩展清单和重点页面七视口适配；v11 为高风险命令加入 TxKernel、持久 Outbox、Worker、命令关联审计、Webhook 安全投递和保留预演。旧 API、旧权限表、旧资源来源和 Manifest 仍通过兼容层读取。
