...
项目当前开发基线为 `v0.6.24-dev`，v6 计划内工作已完成并封版。v0.6 已完成路由级 API/授权契约、三轴插件生命周期、Extension Gateway、`campusos.ui/v1` 动态前端运行时、全屏 `campusos.app-style-pack.v2`、风格包动态配置 schema、数据库约束、插件 SDK/工具链、真实浏览器工作流和单节点恢复门禁；gRPC 正式 ExtensionService、标准 MCP/真实 IM adapter 与多节点运行时属于下一版本候选，不计入 v6 未完成项。
