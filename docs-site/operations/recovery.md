...
备份包含 PostgreSQL dump、migration、插件代码、插件数据和版本快照、个人空间、图片及本地配置，并为内部文件生成 SHA-256 清单。备份可能包含密钥，不得上传公共仓库。
...
演练创建临时数据库，恢复并读取用户、帖子、插件和 migration，再检查插件与个人空间文件；结束后删除临时数据库，不覆盖当前实例。
...
用户级插件在覆盖更新前自动保存到 `data/plugin_data/<plugin>/version-snapshots/`。管理端“插件管理 -> 版本”可以查看经过 checksum 校验的快照并恢复。系统级插件随服务源码部署，只能通过代码版本回退并重启。
