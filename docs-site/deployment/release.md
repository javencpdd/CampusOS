...
CampusOS 当前具备本地开发、跨平台 Docker 开发、前端生产构建、后端二进制构建和经过仓库门禁验证的
单主机 Docker Compose 交付。该交付不是高可用集群方案；公网部署仍需要 TLS、外部密钥管理、异机备份、
真实监控和目标环境安全加固。完整步骤见 [Docker 单主机部署与迁移](/deployment/docker)。
