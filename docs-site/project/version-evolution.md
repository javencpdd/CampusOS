...
CampusOS 的版本计划是架构决策和验收历史，不是把所有旧愿景同时保留为当前承诺。当前应用启动/合同版本是
`v0.13.0`；v0.14-dev 已确认开发收尾。v1.0 已进入开发，当前完成 clean database foundation。
...
| v1.0 | 插件生态与三层授权 | clean database foundation 已实施；统一授权服务、Grant/Consent UI、Host API v3 和工具链仍按计划推进 |
...
- 当前数据库使用 `000001-000003` clean baseline，包含 84 张业务表和 2 张 migration 系统表；静态旧表清单不能替代 Admin 只读架构和数据库检查器。
