...
| Host API | v0.3-dev 已扩展 | 支持读取用户/主题/回复、查询主题、发布事件、发送通知、配置读写、权限检查、插件日志、插件 KV 存储等能力 |
| Go SDK | v0.3-dev 雏形完成 | `sdk/go` 提供 Host API HTTP Client、事件和 manifest 类型 |
| CLI | v0.3-dev 雏形完成 | `cmd/campusosctl` 支持 `plugin init` 和 `plugin inspect` |
...
| [`docs/进度/v0.3-dev-pre/前置稳定化完成说明.md`](./docs/进度/v0.3-dev-pre/前置稳定化完成说明.md) | v0.3-dev 前置稳定化总结 |
...
| `GetConfig` | 读取插件配置 |
| `SetConfig` | 更新运行期插件配置，目前尚未持久化 |
| `CheckPermission` | 调用宿主 RBAC 检查 |
| `Log` | 写入 `plugin_logs` |
| `StorageGet` / `StorageSet` / `StorageDelete` | 插件 KV 存储接口，目前是最小实现 |
...
当前 CLI 是 v0.3-dev 雏形，后续可继续扩展 `plugin pack`、`plugin install`、`plugin test` 等能力。
...
| 项目 | 当前限制 | 后续方向 |
| --- | --- | --- |
| `SetConfig` | 目前只更新运行期内存 manifest | 接入数据库或配置仓储，并增加审计 |
| 插件 KV 存储 | 当前是最小接口和内存实现 | 按 SQLite 存储设计落地持久化 |
| 插件打包 | 已有规范，CLI 尚未实现 pack/install | 增加 `campusosctl plugin pack/install` |
| Go SDK | 已封装 Host API 常用方法 | 增加本地测试 harness、Wasm/TinyGo 模板 |
