...
截至当前代码与文档状态，`v0.5-dev` 任务已经完成到 `docs/进度/v0.5-dev/v0.5.18-dev.md`。README 只记录已经落地或当前可验证的能力；真实 IM 平台适配、标准 MCP 协议适配器、完整插件市场、完整 Docker 产品化封装、原生 Windows 实机兼容和 AI 内容审核插件仍属于后续或暂缓事项。
...
| 管理后台 `admin/` | 已实现并持续增强 | 默认端口 `3001`，包含用户、帖子、版块默认标签、插件配置、说明文档、AI、事件、平台日志、集成中心、Webhook、MCP 工具和 Message local 测试入口 |
...
| 个人主页/风格包/个人空间文件 | 已实现核心闭环 | 支持公开个人主页、内容同步、JSON 风格包、标准目录文件夹/zip 拓展风格包、受限 HTML/CSS 风格、预览、应用、回滚、恢复默认、头像上传和默认 10MB 本地个人空间 |
...
| [v5 版本计划书第二版](./docs/项目计划v5/01-v5版本计划书第二版.md) | 基于 v0.5.18-dev 的审查优化版和后续收口路线 |
| [v0.5-dev 进度目录](./docs/进度/v0.5-dev/) | v0.5.0 到 v0.5.18 实施记录 |
