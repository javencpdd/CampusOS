...
截至当前代码与文档状态，`v0.5-dev` 任务已经完成到 `docs/进度/v0.5-dev/v0.5.28-dev.md`。README 只记录已经落地或当前可验证的能力；真实 IM 平台适配、标准 MCP 协议适配器、完整插件市场、完整 Docker 产品化封装、原生 Windows 实机兼容和 AI 内容审核插件仍属于后续或暂缓事项。
...
| 个人课表插件 | 已实现 MVP 闭环 | 内置 `personal-schedule` 插件支持春/秋学期与年份、第一周开始日期、周课表、可浏览过去和未来月份的日历、手动编辑、JSON 编辑、`.xls` / `.csv` / `.json` 导入，并将课表 JSON 存入个人空间文件目录 |
| 插件包治理 | 已实现 v0.5 闭环 | 支持 `campusosctl plugin init/inspect/pack/install`、后台导入导出、配置表单、预检、checksum、包大小、权限风险分级、版本比较、导入审计和手动回滚说明 |
| 受控富文本图文文章 | 已实现 MVP 闭环 | 内置 `controlled-richtext-article` 插件支持图文草稿、编辑、图片上传、HTML 清洗、预览、发布、详情渲染、作者下架/删除和管理员治理；文章图片保存到作者个人空间并计入配额 |
| 个人主页/风格包/个人空间文件 | 已实现核心闭环 | 支持公开个人主页、内容同步、JSON 风格包、标准目录文件夹/zip 拓展风格包、源码目录风格包扫描列表、受限 HTML/CSS 风格、预览、应用、个人主页回滚、首页风格回滚、恢复默认、头像、课表和富文本文章图片的统一个人空间存储 |
...
| [v5 版本计划书第二版](./docs/项目计划v5/01-v5版本计划书第二版.md) | 基于 v0.5.28-dev 的审查优化版和收口结论 |
| [v0.5-dev 进度目录](./docs/进度/v0.5-dev/) | v0.5.0 到 v0.5.28 实施记录 |
...
| 文件夹/zip 拓展风格包 | 已实现，标准为 `page-style-pack.v1`，支持 personal-space 和 homepage 两类 target，源码包包含 `templates/`、`assets/`、`styles/`、`preview.png` 和 `config.schema.json` |
...
| 学年与学期 | 已实现，可指定课表对应年份及春季或秋季学期 |
| 第一周开始日期 | 已实现，可设置该学期第一周的开始日期，用于计算周次与日期范围 |
| 周课表 | 已实现，按星期和节次展示当前选择周课程 |
| 日历浏览 | 已实现，可切换任意过去或未来月份；课程按第一周日期、星期和周次显示 |
| 手动编辑课程 | 已实现，支持课程名称、教师、地点、星期、节次、周次、颜色和备注 |
| 高级 JSON 编辑 | 已实现，可直接编辑课表数据结构 |
| 表格导入 | 已实现，支持 `.xls` 示例课表、`.csv` 和 `.json` |
| 个人空间存储 | 已实现，课表保存到 `data/personal-space/<user_id>/file/schedule/schedule.json`；头像保存到 `img/avatars/` |
...
| 开发者说明文档 | 已实现管理端入口 | 管理后台 `/docs` 提供新开发者上手、插件编写、插件管理和排查说明 |
...
│   ├── images/                    # 非个人空间的全局图片资源预留目录
│   ├── personal-space/            # 按用户分类保存头像、课表和图文文章图片
│   ├── dist/                      # 本地发布产物预留目录
│   ├── config/                    # 本地运行配置预留目录
