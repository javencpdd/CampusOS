...
description: Audit and synchronize CampusOS migrations, system-managed schema tables, persistent data boundaries, and the Admin data-architecture view. Use when migrations or migrate scripts change, when checking migration redundancy or rollback safety, when storage/plugin data layout changes, or when auditing `admin/src/modules/architecture/pages/SystemArchitectureView.vue` and the Admin `/architecture` page.
---
...
2. Read the changed migration and the matching data model or service. When the task asks about redundancy, ordering,
   rollback, or applied migration edits, read [migration safety contract](references/migration-safety-contract.md) before
   deciding whether a new forward migration is required. Then update
...
6. Update `docs/architecture/当前架构概览.md`, relevant API/Help or official docs, the current progress document, and
   project onboarding status when schema or the user-visible architecture view changes.
...
- `references/migration-safety-contract.md`: defines immutability, redundancy review, database drills, data-preserving
  rollback, and documentation expectations.
