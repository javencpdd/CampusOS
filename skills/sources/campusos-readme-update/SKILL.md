...
Use the active CampusOS workspace as the repository root. When invoked from another directory, derive the root from this
Skill's `skills/sources/campusos-readme-update/` location; do not assume a Linux home path.
...
find docs/项目计划书v* -mindepth 2 -maxdepth 2 -type f | sort -V
```
...
- `skills/guides/`: Skill usage, maintenance, validation, and repository discovery.
...
- `docs/项目计划书v<major>/项目计划v<version>/`: planned scope, sequencing, acceptance, and deferred work.
...
2. Add it to the nearest category index, such as `skills/guides/README.md`, when one exists.
...
```bash
python3 skills/sources/campusos-readme-update/scripts/audit_readme_structure.py \
  --root . \
