...
| `current-project-status.md` | `v0.13.0` 发布能力、v0.14 当前实施状态、限制、目录和运行快照 |
...
- migration `000001-000049`、v0.14 正式计划、最新 `v0.14.11-dev` 开发收尾记录、`v0.14.9-dev` 逐项审查/可恢复对账/发布边界记录，以及关键代码/数据目录。
- 原生开发、Docker 开发、总编排和分组件启动方式。
- 当前仍未实现的标准 MCP、标准 protobuf gRPC、远程市场、真实第三方 Adapter 和高可用。
- Help 文档生命周期、v0.1-v0.14 计划总结和当前 v0.14 正式路线。
...
```bash
./skills/sources/campusos-project-onboarding/scripts/context_snapshot.sh
```
...
campusos-project-onboarding
  ...
  -> Docker 问题使用 campusos-docker-development
  -> WebUI 问题使用 campusos-webui-regression
  -> 按影响运行测试、构建、架构和发布门禁
  -> 更新唯一权威文档和进度证据
```
...
```bash
bash -n skills/sources/campusos-project-onboarding/scripts/context_snapshot.sh
./skills/sources/campusos-project-onboarding/scripts/context_snapshot.sh
python <skill-creator目录>/scripts/quick_validate.py skills/sources/campusos-project-onboarding
git diff --check
```

仓库副本是版本化事实源，`.agents/skills/campusos-project-onboarding/` 是可提交的发现桥接。clone 后从仓库内
启动 Codex 即可直接调用，不需要安装到用户目录；若已打开的会话未刷新，重启 Codex 一次。
