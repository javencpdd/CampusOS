...
- 审查 migration 是否冗余、是否错误修改历史文件、up/down 是否安全或是否需要新的前向修复 migration。
...
涉及 migration 安全审查时还要读取
`skills/sources/campusos-data-architecture-sync/references/migration-safety-contract.md`，并按影响运行
`./scripts/database-check.sh all`、`make v12-migration-check`、`make v13-migration-check`。已经可能在开发或交付
环境执行的编号 migration 视为不可变历史；修复使用新的前向 migration，不能为了让当前 schema 好看而原地改写。
