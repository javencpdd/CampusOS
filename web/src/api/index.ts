...
export const richTextApi = {
  ...
  uploadAsset: (file: File, data?: { thread_id?: string; article_content_id?: string }) => {
    ...
    return api.post('/richtext/assets', form, {
...
export const scheduleApi = {
  ...
  import: (file: File, term: { term_year: number; semester: 'spring' | 'fall' }, replace = false) => {
    ...
    return api.post('/schedule/me/import', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 60000,
    })
  },
...
export const spaceApi = {
  ...
  validateStylePack: (file: File) => {
    const form = new FormData()
    form.append('file', file)
    return api.post('/spaces/me/styles/packs/validate', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  },
  stylePackExample: () => api.get('/spaces/me/styles/packs/example'),
  stylePackExampleZip: () =>
    api.get('/spaces/me/styles/packs/example.zip', {
      responseType: 'blob',
      timeout: 60000,
    }) as Promise<Blob>,
  ...
  applyStylePack: (file: File) => {
    const form = new FormData()
    form.append('file', file)
    return api.post('/spaces/me/styles/packs/apply', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  },
  ...
  uploadAvatar: (file: File) => {
    const form = new FormData()
    form.append('file', file)
    return api.post('/spaces/me/avatar', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  },
