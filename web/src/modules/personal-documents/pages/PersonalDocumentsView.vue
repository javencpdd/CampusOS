<template>
  <section class="documents-view" v-loading="loading">
    ...
    <el-dialog v-model="editorVisible" :title="editor.name || '文档'" width="min(760px, 94vw)"><template v-if="editable"><el-input v-model="editor.name" maxlength="255" /><el-input v-model="editor.content" type="textarea" :rows="18" class="content" /><el-button @click="showVersions">版本历史</el-button></template><el-alert v-else type="info" :closable="false" title="该格式不可在线编辑，请下载后在本地打开。" /><template #footer><el-button @click="editorVisible=false">关闭</el-button><el-button v-if="editable" type="primary" :loading="saving" @click="save">保存为新版本</el-button></template></el-dialog>
...
const editor=ref<any>({}), editable=computed(()=>['text','markdown','campusdoc'].includes(editor.value.format))
...
function createDocument(){editor.value={name:'未命名文档.txt',format:'text',content:'',version:0,id:''};editorVisible.value=true}
...
<style scoped>
.documents-view{max-width:1200px;margin:0 auto;padding:20px}.header{display:flex;justify-content:space-between;gap:16px;align-items:center}.header h2{margin:0 0 6px}.header span{color:var(--el-text-color-secondary);font-size:13px}.toolbar{display:flex;gap:8px;margin-bottom:14px}.hidden{display:none}.content{margin-top:12px}@media(max-width:600px){.documents-view{padding:10px}.header{align-items:flex-start}.header span{display:none}}
</style>
