# v13 立项输入中的历史讨论（非正式计划）

> 文档生命周期：原始对话摘录，主体内容实际讨论 v12 可信账号和结构化社区。
> v13 当前范围只以 [正式计划](00-v13版本计划书.md) 和
> [最终专业审计](02-v13最终专业审计与后续路线.md) 为准。

message time: 2026-07-17 00:57:33

请结合当前项目仓库代码，再次检查v12计划书，对其进行深度检查与优化

---

# chatgpt response

我会先核对仓库最新主分支、迁移编号、Identity、Category、Thread、RichText 与 Reliability 的当前实现，再逐项验证 v12 计划是否与现有边界冲突。
...
当前 `main` 最新提交为 `cb128267...`，该提交只修改了文档思考文件，没有改变核心业务代码；运行时版本仍为 `0.11.0`。因此，v12 可以继续以已经完成的 v11 Reliability 基线为起点。  

上一版 v12 的总体方向正确，但结合代码后需要进行八项关键修正：
...
因此，v12 不应再增加第三套“个人邮箱绑定表”。优化后的所有权应为：
...
v12 不应实现 QQ、Gmail 的点号合并或 `+alias` 折叠，避免错误地把不同地址视为同一个邮箱。
...
`users.email` 在 v12 中只是兼容投影，后续版本可逐渐从 User 领域实体中移除。
...
当前 `Register` 已经接入 `reliability.Execute`，v12 应扩展已有命令，而不是另建第二套注册入口。
...
当异常注册量明显上升后，再增加人机验证；不应把第三方验证码作为 v12 的强制依赖。
...
当前 v11 的可靠性迁移为 `000027`，因此 v12 从 `000028` 开始。
...
| 阶段 | 内容 | 出口 |
|---|---|---|
| A0 | 基线修复、Color 合同、默认管理员安全、文档元数据 | v11 基线稳定 |
...
v12 应改为读取统一 Version，或输出不带固定版本的文本。
...
该版本可作为正式 v12 计划书的替换基线；相较上一版，邮箱事实、Token 失效、邮件模块边界和结构化帖子事务方式已经与当前仓库实现对齐。
