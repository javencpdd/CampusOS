...
状态和不含敏感数据的可操作错误分类，也可读取：

```text
GET /api/v1/platform/email-delivery/status
```

响应不会包含 SMTP host、用户名、收件人、验证码、Ticket、正文或 Provider 原始错误。发生故障时会区分
连接失败、TLS 配置错误、身份验证失败、发件地址被拒绝、收件地址被拒绝和邮件提交失败，并给出中文修复建议；
可靠任务仍只保存统一脱敏错误，避免第三方 SMTP 响应意外带出地址或凭据。

## 5. 排障顺序

1. 查看可靠任务中的 `identity.email.challenge.requested.v1` 是否为 `retry` 或 `dead`。
2. 查看邮件投递状态是否为 `degraded`。
3. 按邮件投递卡片的中文分类检查网络/TLS、SMTP 授权码、发件人与收件地址；QQ/Foxmail 的 `535` 通常表示授权码无效、已撤销或未启用 SMTP 服务，而不是 Docker 网络故障。
