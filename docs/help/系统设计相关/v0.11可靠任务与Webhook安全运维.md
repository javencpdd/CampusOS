...
[v0.12 可靠事件异常诊断与安全恢复](v0.12可靠事件异常诊断与安全恢复.md)。
...
`make database-check` 检查当前库的审计、Schema 和迁移卫生；当前迁移链只有 `000001` 至 `000003`。
旧 v0.11 的 `000027` 演练脚本不属于当前数据库门禁，不能在现行库上执行。

| 现象 | 优先检查 | 不要做 |
| --- | --- | --- |
| queue 持续积压 | Worker 心跳、`make migrate-status`、数据库连接、应用日志 | 不要清空 Outbox。 |
...
详见 [可靠命令与数据所有权](../../architecture/v0.11可靠命令事件与数据所有权.md) 和
[Webhook 安全模型](../../architecture/v0.11Webhook可靠投递与安全模型.md)。
