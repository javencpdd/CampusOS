...
| 管理员日志接口 | 已提供日志查询 |
| 管理后台页面 | 未完成 |
| 数据库日志持久化 | 已提供 PG 模式下的 `ai_call_logs` |
| Host API 暴露给插件 | 未完成 |
...
PostgreSQL 可用时，调用日志会写入 `ai_call_logs`；服务回退到内存模式时，日志只保存在内存中，服务重启后会丢失。
...
| `model` | 调用模型 |
| `source` | 调用来源 |
| `status` | `success`、`error` 或 `rate_limited` |
| `duration_ms` | 调用耗时 |
| `prompt_tokens` | 输入 token 数 |
| `completion_tokens` | 输出 token 数 |
| `total_tokens` | 总 token 数 |
| `error_message` | 错误信息，密钥已脱敏 |
