...
资源包不得包含 `plugin.yaml`、migration、服务端程序或启动脚本，不得获得数据库、JWT、内部 Service 和任意文件系统权限。v0.10 已将仓库内首页、个人主页和系统主题包统一迁入 `data/resources/{homepage-packs,space-style-packs,themes}`；`data/plugin_data` 只保留 External Plugin 运行数据。
