...
资源包不得包含 `plugin.yaml`、migration、服务端程序或启动脚本，不得获得数据库、JWT、内部 Service 和任意文件系统权限。当前新包统一放在 `data/resources/{homepage-packs,space-style-packs,themes}` 等受管目录；`data/plugin_data` 主要保存 External Plugin 运行数据，旧 `style-packs` 仍可被 Legacy Resource Source 兼容读取，但不作为新包写入目标。
...
从 v0.10 旧布局迁移时使用 `scripts/migrate-v10-module-plugin-layout.sh`；脚本遇到同名目标会停止，不覆盖文件，并可按状态记录逆序回滚。
