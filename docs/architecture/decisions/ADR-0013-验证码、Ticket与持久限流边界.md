...
> 适用性说明更新：2026-09-14（Asia/Shanghai）。本 ADR 保留原始决策与当时实施状态，不是当前操作指南。
> 不因版本较旧自动废止安全原则，也不将旧迁移编号、路径和验收结果视作当前事实。
> 当前实现请阅读 [身份认证与权限管理](../模块设计/身份认证与权限管理.md) 与 [当前架构概览](../当前架构概览.md)。
